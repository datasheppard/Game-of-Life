...
class GameOfLife(Frame):
	...
	def build_grid(self):
		...
		self.cell_buttons = [[Button(self.game_frame, bg = "black", width = 2, height = 1) for i in range(self.size_x + 2)] for j in range(self.size_y + 2)]
	...
	def simulate_game(self):
		...
		for i in range(1, self.size_y + 1):
			for j in range(1, self.size_x + 1):
				...
				if self.cell_buttons[i][j]['bg'] == "black" and self.neighbor_count(i, j) == 3:
				...
				elif self.cell_buttons[i][j]['bg'] == "white" and self.neighbor_count(i, j) != 3 and self.neighbor_count(i, j) != 2:
					buttons_to_toggle.append(coord)
	...
	def enable_buttons(self):
		...
		for i in range(0, self.size_y + 2):
			for j in range(0, self.size_x + 2):
				self.cell_buttons[i][j]['bg'] = "black"
	...
	def neighbor_count(self, x_coord, y_coord):
		...
		for i in range(x_coord - 1, x_coord + 2):
			for j in range(y_coord - 1, y_coord + 2):
				if (i != x_coord or j != y_coord) and self.cell_buttons[i][j]['bg'] == "white":
					count += 1
